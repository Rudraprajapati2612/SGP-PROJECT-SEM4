...
export function LightBillsModal({ lightBills: initialBills = [], onClose }) {
  ...
  useEffect(() => {
    const fetchBills = async () => {
      try {
        const response = await fetch("http://localhost:3000/api/AddLightBill", { method: "GET" })
        if (response.ok) {
  ...
  const handleMarkAsPaid = (id) => {
    setLightBills((prevBills) =>
      prevBills.map((bill) => (bill.id === id ? { ...bill, status: "Paid" } : bill))
    )
  }

  const fetchPreviousUnits = async (roomNumber) => {
    try {
      const response = await fetch(`http://localhost:3000/api/AddLightBill?roomNumber=${roomNumber}`, {
        method: "GET",
      })
      if (response.ok) {
        const data = await response.json()
        ...
        if (previousBill) {
          setNewBill((prev) => ({ ...prev, previousUnit: previousBill.CurrentUnits }))
        } else {
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex space-x-2">
              ...
              <Button className="bg-[#6C5DD3] hover:bg-[#5B4DC3]" onClick={() => setIsAddBillOpen(true)}>
            ...
            <div className="divide-y divide-gray-800 text-white">
              {filteredBills.length > 0 ? (
                filteredBills.map((bill) => (
                  <div key={bill.id} className="grid grid-cols-7 p-3 hover:bg-[#0F1117]/50 text-sm">
                    <div>{bill.room}</div>
                    <div>{bill.month}</div>
                ...
                <div className="p-4 text-center text-gray-400">No bills found. Add a new bill to get started.</div>
              )}
            </div>
          </div>
        </div>
        ...
        <DialogContent className="bg-[#1B1D23] text-white border-gray-800">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Select onValueChange={(value) => setNewBill({ ...newBill, month: value })} value={newBill.month}>
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input
                ...
                onChange={(e) => setNewBill({ ...newBill, previousUnit: Number.parseInt(e.target.value) || 0 })}
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input
                ...
                onChange={(e) => setNewBill({ ...newBill, currentUnit: Number.parseInt(e.target.value) || 0 })}
              />
            </div>

            <div className="grid grid-cols-4 items-center gap-4">
              <div className="text-right text-gray-400">Units Consumed</div>
              <div className="col-span-3">{Math.max(0, newBill.currentUnit - newBill.previousUnit)} units</div>
            </div>

            <div className="grid grid-cols-4 items-center gap-4">
              <div className="text-right text-gray-400">Bill Amount</div>
              <div className="col-span-3">₹{Math.max(0, newBill.currentUnit - newBill.previousUnit) * newBill.rate}</div>
            </div>
          </div>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!newBill.room || !newBill.month || newBill.currentUnit <= newBill.previousUnit}
            >
              Add Bill
            </Button>
          </DialogFooter>
